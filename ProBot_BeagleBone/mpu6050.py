...
class MPU6050:
    ...
    def readACCx(self):
        ...
        accX = (accX_l | accX_h <<8)
            
        return accX  if accX < 32768 else accX - 65536

    def readACCy(self):
        accY_l = bus.read_byte_data(self.MPU6050_DEFAULT_ADDRESS, self.MPU6050_RA_ACCEL_YOUT_L)
        accY_h = bus.read_byte_data(self.MPU6050_DEFAULT_ADDRESS, self.MPU6050_RA_ACCEL_YOUT_H)
        accY = (accY_l | accY_h <<8)

        return accY  if accY < 32768 else accY - 65536

    def readACCz(self):
        accZ_l = bus.read_byte_data(self.MPU6050_DEFAULT_ADDRESS, self.MPU6050_RA_ACCEL_ZOUT_L)
        accZ_h = bus.read_byte_data(self.MPU6050_DEFAULT_ADDRESS, self.MPU6050_RA_ACCEL_ZOUT_H)
        accZ = (accZ_l | accZ_h <<8)

        return accZ  if accZ < 32768 else accZ - 65536

    def readGYRx(self):
        gyrX_l = bus.read_byte_data(self.MPU6050_DEFAULT_ADDRESS, self.MPU6050_RA_GYRO_XOUT_L)
        gyrX_h = bus.read_byte_data(self.MPU6050_DEFAULT_ADDRESS, self.MPU6050_RA_GYRO_XOUT_H)
        GYRx = (gyrX_l | gyrX_h <<8)

        return GYRx  if GYRx < 32768 else GYRx - 65536	
	
    def readGYRy(self):
        gyrY_l = bus.read_byte_data(self.MPU6050_DEFAULT_ADDRESS, self.MPU6050_RA_GYRO_YOUT_L)
        gyrY_h = bus.read_byte_data(self.MPU6050_DEFAULT_ADDRESS, self.MPU6050_RA_GYRO_YOUT_H)
        GYRy = (gyrY_l | gyrY_h <<8)

        return GYRy  if GYRy < 32768 else GYRy - 65536
	
    def readGYRz(self):
        gyrZ_l = bus.read_byte_data(self.MPU6050_DEFAULT_ADDRESS, self.MPU6050_RA_GYRO_ZOUT_L)
        gyrZ_h = bus.read_byte_data(self.MPU6050_DEFAULT_ADDRESS, self.MPU6050_RA_GYRO_ZOUT_H)
    ...
    def MPU6050Values(self):
		...
		AccXangle = ((atan2(accY,accZ)+pi)*RAD_TO_DEG)-180	
													# Calculation of the angle in X axis
